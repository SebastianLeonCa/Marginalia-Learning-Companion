...
router.get("/documents/:documentId/notes", requireAuth, async (req, res): Promise<void> => {
...
router.post("/documents/:documentId/notes", requireAuth, async (req, res): Promise<void> => {
...
router.post("/notes/:noteId/explanation", requireAuth, async (req, res): Promise<void> => {
  ...
  const [note] = await db
    .update(notesTable)
    .set({ explanation: plainLanguageExplanation(existing.selectedText), updatedAt: new Date() })
