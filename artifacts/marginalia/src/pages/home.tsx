...
import { continueReadingCard } from "@/lib/reading-position";
...
function HomeContent() {
  ...
  const continueCard = stats.continueReading
    ? continueReadingCard(stats.continueReading)
    : null;
  return (
    <AppShell>
      <div className="relative mx-auto max-w-6xl px-5 py-8 md:px-10 md:py-12">
        ...
        {stats.continueReading && continueCard && <section className="mt-10 overflow-hidden rounded-3xl bg-secondary p-6 text-secondary-foreground md:p-8"><div className="flex flex-wrap items-center justify-between gap-6"><div><p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.2em] opacity-75"><Flame size={15} /> Pick up where you left off</p><h2 className="mt-3 max-w-xl font-serif text-3xl font-bold">{stats.continueReading.studySetTitle}</h2><p className="mt-2 text-sm opacity-75">{continueCard.detail}</p></div><Link href={continueCard.href} className="inline-flex items-center gap-2 rounded-xl bg-background px-5 py-3 text-sm font-bold text-foreground hover:-translate-y-0.5" data-testid="link-continue-reading">Continue reading <ArrowRight size={16} /></Link></div><div className="mt-7 flex items-center gap-4"><ProgressBar value={stats.continueReading.progress} /><span className="text-sm font-bold">{stats.continueReading.progress}%</span></div></section>}
